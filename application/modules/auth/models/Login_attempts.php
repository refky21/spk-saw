...
class Login_attempts extends CI_Model
{
	...
	function __construct()
	{
		...
		$ci =& get_instance();
		$this->table_login_attempts = $ci->config->item('app_db_table_prefix').$this->table_login_attempts;
		$this->table_captcha = $ci->config->item('app_db_table_prefix').$this->table_captcha;
	}
	...
	function get_attempts_num($ip_address, $login)
	{
		...
		$qres = $this->db->get($this->table_login_attempts);
	...
	{
		$this->db->insert($this->table_login_attempts, array('ip_address' => $ip_address, 'login' => $login));
	}
	...
	function clear_attempts($ip_address, $login, $expire_period = 86400)
	{
		...
		$this->db->delete($this->table_login_attempts);
	}
	
	function insert_captcha($data)
	{
		return $this->db->insert($this->table_captcha, $data);
	}
	
	function delete_captcha($data)
	{
		return $this->db->delete($this->table_captcha, $data);
	}
	
	function count_available_captcha($data)
	{
		$sql = "SELECT COUNT(*) AS count FROM ". $this->table_captcha ." WHERE word = ? AND ip_address = ? AND captcha_time > ?";
